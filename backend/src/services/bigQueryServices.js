...
export class BigQueryService {
  ...
  getBigQueryConfig() {
    ...
    if (credentialsString && credentialsString.trim().startsWith("{")) {
      try {
        ...
        console.error("❌ Erro ao parsear credenciais JSON:", error.message);
        // Fallback: tenta como arquivo
        return {
          projectId,
  ...
  processarEscolaCompleta(escola, ano = "2024") {
    const mapping =
      completeColumnMappings[ano] || completeColumnMappings["2024"];
    ...
    return {
      ...
      oferta_educacional: this.extrairOfertaEducacionalCompleta(
        escola,
        mapping
      ),
      ...
      recursos_pedagogicos: this.extrairRecursosPedagogicosCompleto(
        escola,
        mapping
      ),
  ...
  extrairIdentificacao(escola, mapping) {
    return {
      ...
      categoria_privada: this.mapearCategoriaPrivada(
        escola[mapping.TP_CATEGORIA_ESCOLA_PRIVADA]
      ),
      localizacao_tipo: this.mapearLocalizacao(escola[mapping.TP_LOCALIZACAO]),
      localizacao_diferenciada: this.mapearLocalizacaoDiferenciada(
        escola[mapping.TP_LOCALIZACAO_DIFERENCIADA]
      ),
      situacao: this.mapearSituacaoFuncionamento(
        escola[mapping.TP_SITUACAO_FUNCIONAMENTO]
      ),
    ...
    return {
      dependencias: {
        laboratorio_informatica:
          escola[mapping.IN_LABORATORIO_INFORMATICA] === 1,
        ...
        biblioteca_sala_leitura:
          escola[mapping.IN_BIBLIOTECA_SALA_LEITURA] === 1,
  ...
  extrairTecnologiaCompleta(escola, mapping) {
    return {
      ...
      equipamentos: {
        computadores_alunos:
          (escola[mapping.QT_DESKTOP_ALUNO] || 0) +
          (escola[mapping.QT_COMP_PORTATIL_ALUNO] || 0),
  ...
  criarResumoCompletoParaRAG(escola, mapping, ano) {
    const caracteristicas = this.extrairCaracteristicasPrincipais(
      escola,
      mapping
    );
  ...
  extrairCaracteristicasPrincipais(escola, mapping) {
    ...
    // Infraestrutura
    if (escola[mapping.IN_LABORATORIO_INFORMATICA] === 1)
      caracteristicas.push("Laboratório de Informática");
    if (escola[mapping.IN_BIBLIOTECA] === 1) caracteristicas.push("Biblioteca");
    if (escola[mapping.IN_QUADRA_ESPORTES] === 1)
      caracteristicas.push("Quadra de Esportes");
    if (escola[mapping.IN_INTERNET] === 1)
      caracteristicas.push("Acesso à Internet");
    if (escola[mapping.IN_REFEITORIO] === 1) caracteristicas.push("Refeitório");

    // Recursos especiais
    if (escola[mapping.IN_EDUCACAO_INDIGENA] === 1)
      caracteristicas.push("Educação Indígena");
    if (escola[mapping.IN_ACESSIBILIDADE_RAMPAS] === 1)
      caracteristicas.push("Acessibilidade (Rampas)");
    if (escola[mapping.IN_BANHEIRO_PNE] === 1)
      caracteristicas.push("Banheiro Acessível");
    if (escola[mapping.IN_ALIMENTACAO] === 1)
      caracteristicas.push("Alimentação Escolar");

    // Modalidades
    if (escola[mapping.IN_EJA] === 1) caracteristicas.push("EJA");
    if (escola[mapping.IN_PROF_TEC] === 1)
      caracteristicas.push("Ensino Profissional Técnico");
  ...
  extrairRecursosPedagogicosCompleto(escola, mapping) {
    return {
      materiais_didaticos: {
        ...
        brinquedos_educacao_infantil:
          escola[mapping.IN_BRINQUEDOS_ED_INFANTIL] === 1,
        materiais_cientificos: escola[mapping.IN_MATERIAL_CIENTIFICO] === 1,
        materiais_educacao_indigena:
          escola[mapping.IN_MATERIAL_ED_INDIGENA] === 1,
        materiais_educacao_etnicorracial:
          escola[mapping.IN_MATERIAL_ED_ETNICORRACIAL] === 1,
  ...
  extrairAcessibilidadeCompleta(escola, mapping) {
    return {
      estrutural: {
        ...
        sinalizacao_sonora:
          escola[mapping.IN_ACESSIBILIDADE_SINAL_SONORO] === 1,
        sinalizacao_tatil: escola[mapping.IN_ACESSIBILIDADE_SINAL_TATIL] === 1,
        sinalizacao_visual:
          escola[mapping.IN_ACESSIBILIDADE_SINAL_VISUAL] === 1,
        vao_livre: escola[mapping.IN_ACESSIBILIDADE_VAO_LIVRE] === 1,
        portas_automaticas:
          escola[mapping.IN_ACESSIBILIDADE_PORTAS_AUTOMATICAS] === 1,
      },
      sanitarios: {
        banheiro_pne: escola[mapping.IN_BANHEIRO_PNE] === 1,
      ...
      recursos_especializados: {
        sala_atendimento_especial:
          escola[mapping.IN_SALA_ATENDIMENTO_ESPECIAL] === 1,
        recursos_educacao_especial:
          escola[mapping.IN_RECURSOS_EDUCACAO_ESPECIAL] === 1,
      },
    };
  }
    ...
    return {
      ...
      esgoto_sanitario: {
        ...
        esgoto_fossa_rudimentar:
          escola[mapping.IN_ESGOTO_FOSSA_RUDIMENTAR] === 1,
      ...
      lixo: {
        ...
        lixo_destino_final_publico:
          escola[mapping.IN_LIXO_DESTINO_FINAL_PUBLICO] === 1,
        lixo_outros: escola[mapping.IN_LIXO_OUTROS] === 1,
      },
      tratamento_agua: {
        tratamento: this.mapearTratamentoAgua(
          escola[mapping.TP_TRATAMENTO_AGUA]
        ),
  ...
  extrairGestaoCompleta(escola, mapping) {
    return {
      ...
      estrutura_gestao: {
        ...
        composicao_conselho: this.mapearComposicaoConselho(
          escola[mapping.TP_COMPOSICAO_CONSELHO]
        ),
      ...
      alimentacao: {
        ...
        tipo_alimentacao: this.mapearTipoAlimentacao(
          escola[mapping.TP_ALIMENTACAO]
        ),
      },
      transporte_escolar: {
        oferta_transporte: escola[mapping.IN_TRANSPORTE] === 1,
        tipo_transporte: this.mapearTipoTransporte(
          escola[mapping.TP_TRANSPORTE]
        ),
      },
    };
  }
